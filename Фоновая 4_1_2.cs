...
namespace Вариант8
{
    class Parallelepiped
    {
        ...
        public int SurfaceS
        {
            get
            {
                return this.SideS + 2 * this.BasisS;
            }
        }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Диагональ параллелепипеда: {0:f2}", first.Diagonal);
            Console.WriteLine("Периметр всех ребр параллелепипеда: {0} cм", first.LengthOfEdges);
            if (first.isCube) is_cube = "да";
            else is_cube = "нет";
            Console.WriteLine("Является ли  параллелепипед кубом: {0}", is_cube);
